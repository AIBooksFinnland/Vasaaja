...
class SyncService {
  ...
  async processReceivedEntry(entry, sourceId) {
    try {
      if (!this.isHost || !entry || !entry.id) return;
  ...
  async sendEntry(entry) {
    try {
      ...
      if (this.connectedPeers.size > 0) {
        ...
        // Send to the first connected peer (in a real app, you'd know which one is the host)
        const firstPeerId = [...this.connectedPeers.keys()][0];
  ...
  async requestFullSync() {
    try {
      if (this.isHost || this.connectedPeers.size === 0) return;
