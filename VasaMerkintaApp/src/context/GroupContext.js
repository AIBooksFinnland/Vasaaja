...
export const GroupProvider = ({ children }) => {
  ...
  const loadUserGroups = async () => {
    ...
    try {
      ...
      // Filter groups where user is member or owner
      const userGroupIds = allGroups
        .filter(g => g.ownerId === user.id || g.members.includes(user.id))
        .map(g => g.id);
  ...
  const createGroup = async (groupName, location) => {
    if (!user) throw new Error('Kirjaudu sisään luodaksesi ryhmän');
    ...
    try {
      ...
      await AsyncStorage.setItem(`userGroups_${user.id}`, JSON.stringify(userGroups));
  ...
  const requestJoinGroup = async (groupId, location) => {
    if (!user) throw new Error('Kirjaudu sisään liittyäksesi ryhmään');
    ...
    try {
      ...
      // Get all groups
      const groupsJson = await AsyncStorage.getItem('groups');
      if (!groupsJson) throw new Error('Ryhmää ei löytynyt');
      
      const allGroups = JSON.parse(groupsJson);
      const groupToJoin = allGroups.find(g => g.id === groupId);
      
      if (!groupToJoin) throw new Error('Ryhmää ei löytynyt');
  ...
  const acceptJoinRequest = async (groupId, userId) => {
    if (!user) throw new Error('Kirjaudu sisään');
    ...
    try {
      ...
      if (!group) throw new Error('Ryhmää ei löytynyt');
  ...
  const addEntry = async (entryData, groupId) => {
    if (!user) throw new Error('Kirjaudu sisään lisätäksesi merkinnän');
